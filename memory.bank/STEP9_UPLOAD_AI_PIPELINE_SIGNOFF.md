...
Until real AI integration is implemented:
- Step 9 remains ASR-only and produces the transcript.
- Step 10 owns the local AI-result boundary after ASR succeeds.
- The current AI result is generated by a local fake processor, not by network or backend AI.
- Code must keep the real-AI boundary explicit and avoid API keys/backend assumptions.
