...
4. Result persistence fields:
   ...
   - `primaryCategoryId`: populated by the Step 10 diary AI boundary after transcript assembly.
   - `dynamicTags`: populated by the Step 10 diary AI boundary after transcript assembly.
   - `polishedArticle`: populated by the Step 10 diary AI boundary after transcript assembly.

## Temporary AI Placeholder Rule
Current AI boundary:
- Step 9 remains ASR-only and produces the transcript.
- Step 10 owns the AI-result boundary after ASR succeeds.
- The current implementation can call DeepSeek directly from Android for development validation.
- Fake AI remains as a configurable fallback.
- Backend AI job processing remains deferred.
...
## 2026-05-10 Local AI Boundary Update
- Added a minimal local fake AI processor after successful ASR transcript assembly.
- No real AI API, backend job, sync worker, API key, Room schema, Xfyun, or recording-chain changes were introduced.
- The ASR path still ends at transcript production; diary title/category/tags/article shaping happens in the diary assembly boundary.
- Real-device validation confirmed the duplicate tag issue is fixed for:
  `今天很忙碌，项目推进到很晚，一直在开会，整个人很疲惫。`
  Expected result: category `work`, dynamic tag `工作`, with no duplicate `work / 工作` tag chips.

## 2026-05-10 DeepSeek AI Update
- Added real DeepSeek diary AI provider behind the existing `DiaryAiProcessor` boundary.
- Step 9 ASR and recording behavior remain unchanged.
- DeepSeek is only called after ASR transcript success.
- DeepSeek failure, HTTP failure, JSON parse failure, or invalid AI fields still fall back to saving the original transcript.
- DeepSeek API key is read from `local.properties` and must not be committed.
